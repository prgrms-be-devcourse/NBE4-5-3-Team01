...
const FollowPage = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (currentUserId === userIdToUse && activeTab === "following") {
          ...
          response = await axios.get(
            `http://localhost:8080/api/v1/follows/my/pending`,
            {
              withCredentials: true,
            }
          );
          ...
          response = await axios.get(
            `http://localhost:8080/api/v1/follows/${activeTab}/${userIdToUse}`,
            {
              withCredentials: true,
            }
          );
  ...
  const toggleFollow = async (
    ...
    try {
      if (isFollowing == "ACCEPT" || isFollowing == "PENDING") {
        await axios.delete(
          `http://localhost:8080/api/v1/follows/delete/${userId}`,
          {
            withCredentials: true,
            headers: {
  ...
  const rejectFollow = async (e: React.MouseEvent, userId: string) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8080/api/v1/follows/reject/${userId}`,
        {
          withCredentials: true,
        }
      );
  ...
  return (
    <div className="flex flex-col items-center min-h-screen py-10">
      ...
      <div className="flex w-full max-w-lg bg-white rounded-lg shadow-md">
        <button
          className={`w-1/2 py-3 text-lg font-semibold transition-all rounded-l-lg ${
            activeTab === "following"
        ...
        {isCurrentUserProfile && (
          <button
            className={`w-1/2 py-3 text-lg font-semibold transition-all rounded-r-lg ${
              activeTab === "pending"
                ? "bg-[#C8B6FF] text-white shadow"
                : "bg-white text-gray-800 hover:bg-gray-100"
            }`}
              ...
              {activeTab === "pending" ? (
                ...
                <button
                  className={`px-4 py-2 text-sm font-medium rounded-lg transition ${
                    isFollowing
                      ? "bg-[#BBD0FF] text-gray-800 hover:bg-[#B8C0FF]"
                      : isFollower
                      ? "bg-[#FFD6FF] text-gray-800 hover:bg-[#E7C6FF]"
                      : "bg-[#C8B6FF] text-white hover:bg-[#B8C0FF]"
                  }`}
